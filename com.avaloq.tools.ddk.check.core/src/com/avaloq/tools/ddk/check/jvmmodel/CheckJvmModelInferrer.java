...
public class CheckJvmModelInferrer extends AbstractModelInferrer {
  ...
  protected void _infer(final CheckCatalog catalog, final IJvmDeclaredTypeAcceptor acceptor, final boolean preIndexingPhase) {
    ...
    acceptor.<JvmGenericType> accept(jvmTypesBuilder.toClass(catalog, checkGeneratorNaming.qualifiedValidatorClassName(catalog)), (final JvmGenericType it) -> {
      ...
      it.getMembers().add(jvmTypesBuilder.toMethod(catalog, checkGeneratorNaming.fieldGetterName(checkGeneratorNaming.issueCodeToLabelMapFieldName()), issueCodeToLabelMapTypeRef, (final JvmOperation it1) -> {
        it1.setFinal(true);
        jvmTypesBuilder.setBody(it1, (final ITreeAppendable appendable) -> {
          appendable.append("return " + checkGeneratorNaming.catalogClassName(catalog) + "."
              + checkGeneratorNaming.fieldGetterName(checkGeneratorNaming.issueCodeToLabelMapFieldName()) + "();");
        });
      }));
  ...
  private JvmOperation createDispatcherMethod(final CheckCatalog catalog) {
    ...
    return jvmTypesBuilder.toMethod(catalog, "validate", _typeReferenceBuilder.typeRef("void"), (final JvmOperation it) -> {
      ...
      jvmTypesBuilder.setBody(it, (final ITreeAppendable out) -> {
  ...
  private void emitDispatcherMethodBody(final ITreeAppendable out, final CheckCatalog catalog, final JvmTypeReference objectBaseJavaTypeRef) {
     ...
     */
    Iterable<Context> checkContexts = Iterables.<Context> concat(ListExtensions.<Check, EList<Context>> map(catalog.getAllChecks(), (final Check chk) -> chk.getContexts()));
    Iterable<Context> implContexts = IterableExtensions.<Context> filterNull(ListExtensions.<Implementation, Context> map(catalog.getImplementations(), (final Implementation impl) -> impl.getContext()));
    Iterable<Context> allContexts = IterableExtensions.<Context> filter(Iterables.<Context> concat(checkContexts, implContexts), (final Context ctx) -> {
    ...
    for (Context context : allContexts) {
      contextsByCheckType.compute(checkGeneratorExtensions.checkType(context), (final CheckType k, final List<Context> lst) -> lst != null ? lst
          : new java.util.ArrayList<Context>()).add(context);
    }
  ...
  private void emitInstanceOfConditionals(final ITreeAppendable out, final List<Context> contexts, final CheckCatalog catalog, final String baseTypeName) {
    ...
    for (Context context : contexts) {
      contextsByVarType.compute(context.getContextVariable().getType().getQualifiedName(), (final String k, final List<Context> lst) -> lst != null ? lst
          : new java.util.ArrayList<Context>()).add(context);
    }

    /* Ordering for context variable type checks. */
    List<JvmTypeReference> contextVarTypes = ListExtensions.<Context, JvmTypeReference> map(contexts, (final Context x) -> x.getContextVariable().getType());
  ...
  private Iterable<JvmMember> createCheckWithParameters(final Check chk) {
    ...
    // First the class
    JvmGenericType checkClass = jvmTypesBuilder.toClass(chk, StringExtensions.toFirstUpper(chk.getName()) + "Class", (final JvmGenericType it) -> {
      ...
      Iterables.addAll(it.getMembers(), IterableExtensions.<FormalParameter, JvmField> map(IterableExtensions.<FormalParameter> filter(chk.getFormalParameters(), (final FormalParameter f) -> f.getType() != null
          && f.getName() != null), (final FormalParameter f) -> jvmTypesBuilder.toField(f, f.getName(), f.getType(), (final JvmField it1) -> {
            it1.setFinal(true);
          })));
    });
    newMembers.add(checkClass);
    newMembers.add(jvmTypesBuilder.toField(chk, StringExtensions.toFirstLower(chk.getName())
        + "Impl", _typeReferenceBuilder.typeRef(checkClass), (final JvmField it) -> {
          jvmTypesBuilder.setInitializer(it, (final ITreeAppendable appendable) -> {
  ...
  private JvmOperation createCheckCaller(final Context ctx, final Check chk) {
    ...
    return jvmTypesBuilder.toMethod(ctx, functionName, _typeReferenceBuilder.typeRef("void"), (final JvmOperation it) -> {
      ...
      jvmTypesBuilder.setBody(it, (final ITreeAppendable appendable) -> {
        ...
        appendable.append(StringExtensions.toFirstLower(chk.getName()) + "Impl" + ".run" + StringExtensions.toFirstUpper(simpleName1)
    ...
    for (FormalParameter parameter : check.getFormalParameters()) {
      ...
      if (returnType != null && !returnType.eIsProxy()) {
        ...
        members.add(jvmTypesBuilder.toMethod(parameter, checkGeneratorNaming.formalParameterGetterName(parameter), returnType, (final JvmOperation it) -> {
          ...
          jvmTypesBuilder.setBody(it, (final ITreeAppendable appendable) -> {
            appendable.append("return checkConfigurationStoreService.getCheckConfigurationStore(context)." + operation + "(\"" + parameterKey + "\", "
                + javaDefaultValue + ");");
          });
        }));
      } // end if
    } // end for
    members.add(jvmTypesBuilder.toMethod(check, "get" + StringExtensions.toFirstUpper(check.getName())
        + "Message", _typeReferenceBuilder.typeRef(String.class), (final JvmOperation it) -> {
    ...
    if (severityType != null) {
      members.add(jvmTypesBuilder.toMethod(check, "get" + StringExtensions.toFirstUpper(check.getName()) + "SeverityKind", severityType, (final JvmOperation it) -> {
        jvmTypesBuilder.setDocumentation(it, "Gets the {@link SeverityKind severity kind} of check\n<em>" + check.getLabel()
            + "</em>. The severity kind returned is either the\ndefault ({@code " + check.getDefaultSeverity().name()
            + "}), as is set in the check definition, or the\nconfigured value, if existing.\n\n@param context\n         the context object used to determine the current project in\n         order to check if a configured value exists in a project scope\n@return the severity kind of this check: returns the default ("
            + check.getDefaultSeverity().name()
        ...
        jvmTypesBuilder.setBody(it, (final ITreeAppendable appendable) -> {
          appendable.append("final int result = checkConfigurationStoreService.getCheckConfigurationStore(context).getInt(\""
              + CheckPropertiesGenerator.checkSeverityKey(check) + "\", " + check.getDefaultSeverity().getValue()
          ...
          for (Check c : allChecks) {
            for (FormalParameter parameter : c.getFormalParameters()) {
              if (parameter.getRight() != null) {
                ...
                String defaultFieldName = CheckGeneratorExtensions.splitCamelCase(checkGeneratorNaming.formalParameterGetterName(parameter)).toUpperCase()
                    + "_DEFAULT";
                ...
                if (typeName != null && typeName.startsWith("java.util.List<")) {
                  ...
                  if (args != null && args.size() == 1) {
                    ...
                    appendable.append("preferences.put(\"" + key + "\", com.avaloq.tools.ddk.check.runtime.configuration.CheckPreferencesHelper.marshal"
                        + baseTypeName + "s(" + defaultFieldName + "));\n");
                  } else {
