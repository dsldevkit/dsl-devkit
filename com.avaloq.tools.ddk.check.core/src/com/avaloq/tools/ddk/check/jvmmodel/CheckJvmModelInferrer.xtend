...
class CheckJvmModelInferrer extends AbstractModelInferrer {
  ...
  def dispatch infer(CheckCatalog catalog, IJvmDeclaredTypeAcceptor acceptor, boolean preIndexingPhase) {
    ...
    acceptor.accept(catalogClass, [
      ...
      val singletonTypeRef = checkedTypeRef(catalog, typeof(Singleton));
      if (singletonTypeRef != null) {
        val singleton = typesFactory.createJvmAnnotationReference();
        val annotationType = singletonTypeRef.type;
        singleton.annotation = annotationType as JvmAnnotationType;
    ...
    acceptor.accept(catalog.toClass(catalog.qualifiedValidatorClassName), [
      ...
      documentation = '''
        Validator for «catalog.name».«IF included != null && !included.eIsProxy» Includes validations from its parent catalog.

        @see «included.validatorClassName»«ENDIF»''';
      ...
      if (included != null && !included.eIsProxy) {
        members += createInjectedField(catalog, included.catalogInstanceName, checkedTypeRef(catalog, included.qualifiedCatalogClassName));
      }
      // Create fields
  ...
  private def void setCheckComposition(CheckCatalog catalog, JvmGenericType jvmType) {
    ...
    val composedCheck = typesFactory.createJvmAnnotationReference();
    val annoTypeRef = checkedTypeRef(catalog, typeof (ComposedCheckValidator));
    if (annoTypeRef == null) {
      return;
    }
    val annotationType = annoTypeRef.type;
    composedCheck.annotation = annotationType as JvmAnnotationType;
    ...
    val JvmTypeReference typeRef = checkedTypeRef(catalog, included.qualifiedValidatorClassName);
    if (typeRef == null) {
      return;
    }
    ...
    annotationValue.operation = (annotationType as JvmDeclaredType).findAllFeaturesByName('validators').head as JvmOperation;
    composedCheck.values += annotationValue;
    // Now we have the equivalent of @ComposedCheckValidator(validators = {<includedCatalogs>.class})
    jvmType.annotations+= composedCheck;
  }
  ...
  private def Iterable<JvmField> createInjectedField(CheckCatalog context, String fieldName, JvmTypeReference type) {
    ...
    val injection = typesFactory.createJvmAnnotationReference();
    val annotationTypeRef = checkedTypeRef(context, typeof(Inject));
    if (annotationTypeRef != null) {
      val annotationType = annotationTypeRef.type;
      injection.annotation = annotationType as JvmAnnotationType;
      field.annotations += injection;
    }
  ...
  private def Iterable<JvmAnnotationReference> createCheckAnnotation (Context ctx) {
    val check = typesFactory.createJvmAnnotationReference();
    val annotationTypeRef = checkedTypeRef(ctx, typeof(org.eclipse.xtext.validation.Check));
    if (annotationTypeRef == null) {
      return Collections::emptyList;
    }
    ...
    val annotationType = annotationTypeRef.type;
    check.annotation = annotationType as JvmAnnotationType;
    ...
    check.explicitValues += memberCall.toJvmAnnotationValue();
  ...
  private def Iterable<JvmMember> createPreferenceInitializerMethods(CheckCatalog catalog) {
    ...
    if (prefStore != null) {
      result += catalog.toMethod('initializeDefaultPreferences', typeRef('void')) [
        val annotationTypeRef = checkedTypeRef(catalog, typeof(Override));
        if (annotationTypeRef != null) {
          val annotationType = annotationTypeRef.type;
          val overrideAnnotation = typesFactory.createJvmAnnotationReference();
