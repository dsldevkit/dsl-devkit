...
public class CheckGenerator extends JvmModelGenerator {
  ...
  public void doGenerate(Resource resource, IFileSystemAccess fsa) {
    ...
    for (CheckCatalog catalog : catalogs) {
      lfFsa.generateFile(_checkGeneratorNaming.issueCodesFilePath(catalog), compileIssueCodes(catalog));
      lfFsa.generateFile(_checkGeneratorNaming.standaloneSetupPath(catalog), compileStandaloneSetup(catalog));
      ...
      if (config != null && (config.doGenerateDocumentationForAllChecks() || !config.isGenerateLanguageInternalChecks())) {
        ...
        lfFsa.generateFile(_checkGeneratorNaming.docFileName(catalog), CheckGeneratorConstants.CHECK_DOC_OUTPUT, compileDoc(catalog));
      }
    }
  }
