...
public class CheckScopeProvider extends XbaseWithAnnotationsBatchScopeProvider {
  ...
  public EClass classForJvmType(final EObject context, final JvmType jvmType) {
    if (jvmType != null && !jvmType.eIsProxy()) {
      ...
      final String qualifiedPackageName = qualifiedName.substring(0, qualifiedName.lastIndexOf('.'));
      final String packageName = qualifiedPackageName.substring(qualifiedPackageName.lastIndexOf('.') + 1);
  ...
  public EPackage getEPackage(final Resource context, final String name) {
    ...
    final EList<Resource> resources = context.getResourceSet().getResources();
    ...
    final IEObjectDescription desc = globalScopeProvider.getScope(context, EcorePackage.Literals.EPACKAGE__ESUPER_PACKAGE, null).getSingleElement(QualifiedName.create(name));
    if (desc != null) {
      return (EPackage) desc.getEObjectOrProxy();
    }
    final Iterator<IEObjectDescription> descs = descriptionsProvider.getResourceDescriptions(context).getExportedObjects(EcorePackage.Literals.EPACKAGE, QualifiedName.create(name), false).iterator();
    if (descs.hasNext()) {
      final EObject pkg = EcoreUtil.resolve(descs.next().getEObjectOrProxy(), context);
    ...
    for (final String nsUri : Sets.newHashSet(EPackage.Registry.INSTANCE.keySet())) {
