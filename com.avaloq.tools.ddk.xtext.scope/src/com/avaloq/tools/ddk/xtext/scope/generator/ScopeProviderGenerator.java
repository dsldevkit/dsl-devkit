...
class ScopeProviderGenerator {
   ...
   */
  def void configure(ScopeNameProviderGenerator nameProviderGenerator, GenModelUtilX genModelUtil, ScopeModel model) {
   ...
   */
  def guardExpression(ScopeRule r) {
    val contextType = r.context.contextType ?: r.scopeType()
    compiler.javaExpression(r.context.guard, translator.newCompilationContext('ctx', contextType, #[], r.context.guard))
  }
   ...
   */
  def stringExpression(Expression expr, EClass contextType) {
    compiler.javaExpression(expr, translator.newCompilationContext('ctx', contextType, #[], expr))
  }
    ...
    try {
      «IF it.exists(r|r.context.guard !== null)»
        «FOR r : it.sortBy(r|if (r.context.guard === null) it.size else it.indexOf(r)) SEPARATOR ' else '
          »«IF r.context.guard !== null»if («guardExpression(r)») «ENDIF»{
  ...
  def dispatch scopeExpression(FactoryExpression it, ScopeModel model, String typeOrRef, ScopeDefinition scope, Boolean isGlobal) {
    ...
    if (method === null) {
      error('cannot resolve scope factory ' + it.expr?.toString
  ...
  def query (GlobalScopeExpression it, ScopeModel model, String typeOrRef, ScopeDefinition scope) '''
    ...
    IF !matchData.isEmpty»«FOR d : matchData».data("«Strings.convertToJavaString(d.key)»", «stringExpression(d.value, eContainer(ScopeRule).context.contextType)»)«ENDFOR»«ENDIF»«
  ...
  def dispatch scopeExpressionPart (GlobalScopeExpression it, ScopeModel model, String typeOrRef, ScopeDefinition scope) '''
    ...
    ENDFOR»)«
    ELSEIF prefix !== null», «stringExpression(prefix, eContainer(ScopeRule).context.contextType)», «recursivePrefix»«
    ENDIF
  »'''
  ...
  def lambdaDataMatch(LambdaDataExpression d, ScopeModel model, EClass contextType, GlobalScopeExpression owner) {
    ...
    if (compiler.isCompilable(d.value, cc)) {
