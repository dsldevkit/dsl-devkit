 ...
 * <p>
 * Translation happens against a {@link ScopeTranslationContext} that provides the variables in scope and the
 * implicit ({@code this}) variable. Increments covered so far:
 * <ul>
 * <li>I1 - literals, list literals and conditional expressions (no JVM linking required).</li>
 * <li>I2 - variable and {@code this} feature calls, linked to the enclosing operation's formal parameters.</li>
 * <li>I3 - boolean, equality and relational operations, linked to the matching {@code org.eclipse.xtext.xbase.lib}
 * operator methods.</li>
 * <li>I3b - getter navigations, linked to the {@code getX()}/{@code isX()} operation on the receiver's JVM type.</li>
 * <li>I4 - extension operation calls, linked to the {@code static} method of an extension class declared through
 * {@code extension a::b::C}.</li>
 * <li>I5 - type casts, {@code typeSelect} filters, {@code isInstance} checks and receiver/implicit method calls,
 * linked to the resolved JVM type, method or {@code com.google.common.collect.Iterables} operation.</li>
 * </ul>
...
public class ScopeExpressionTranslator {
  ...
  /** Memoizes the model type resolver of a scope model for the generation pass. */
  @Inject
   ...
   * against it rather than against the model an individual expression happens to be declared in: a scope provider is
   * generated for one {@code .scope} file, and the rules it inherits from the scope models it includes are compiled
  ...
  /**
   * Returns the scope model an expression must be resolved against: the model currently being generated when one is
   * configured, otherwise the model containing the expression.
   *
   * @param sourceElement
  ...
  /**
   * Resolves a factory expression to the fully qualified static Java method {@code <type qualified name>.<method>}.
   * Two forms are supported, replacing the legacy {@code .ext} based resolution:
   * <ul>
   * <li>{@code method(args)} - the declaring class is one of those declared through {@code extension a::b::C}, the
   * same way ordinary extension calls are resolved;</li>
   * <li>{@code Type.method(args)} - the declaring class is named explicitly and linked against the classpath.</li>
   * </ul>
   *
   * @param expression
  ...
  /**
   * Resolves an unqualified {@code factory method(args)} call to the {@code static} method of one of the extension
   * classes declared through {@code extension a::b::C}. Matching is by name only: the generated call prepends the
   * scope, context, type/reference and original resource arguments, so the number of arguments written in the scope
   * source does not correspond to the method's parameter count.
   *
   * @param call
   *          the unqualified factory operation call, must not be {@code null}
   * @param sourceElement
   *          a model element used to resolve the extension classes against the classpath, must not be {@code null}
  ...
  /**
   * Tests whether the class with the given name declares a {@code static} operation of the given name.
   *
   * @param className
   *          the fully qualified name of the class, must not be {@code null}
   * @param operationName
  ...
  /**
   * Registers the extension classes declared through {@code extension a::b::C} by the scope model containing the
   * given source element (including those of its included scope models), so that calls to their {@code static}
   * methods can be linked. Without this the extension calls would neither be translatable to Xbase nor be qualified
   * by the fallback {@link ScopeExpressionCompiler}.
   *
   * @param context
   ...
   * {@link ScopeExpressionCompiler}. The context binds the implicit ({@code this}) receiver to the given Java
  ...
  /**
   * Returns the model type resolver of the given scope model. The resolver is immutable and determined by the model and
   ...
   */
  private ScopeModelTypeResolver modelTypeResolver(final ScopeModel model) {
    return generatorSupport.memoize(new ModelTypeResolverKey(model), () -> new ScopeModelTypeResolver(model));
  }

  /**
   * Key under which the model type resolver of a scope model is memoized.
   *
   * @param model
   *          the scope model
   */
  private record ModelTypeResolverKey(ScopeModel model) {
  }
   ...
   * receiver (for instance {@code getTarget(SelectionSet)} next to {@code BdeField.getTarget()}) from shadowing it.
   *
   * @param className
   ...
   * about them. Both directions are accepted: the resolved receiver type of a scope expression is often only an
   ...
   * the given call. Used by the fallback {@link ScopeExpressionCompiler} to qualify extension calls that are
