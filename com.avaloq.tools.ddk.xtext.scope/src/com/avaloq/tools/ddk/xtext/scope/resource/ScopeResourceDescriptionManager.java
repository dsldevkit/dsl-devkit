...
/**
 * Resource description manager for scope models which records the classes named in {@code extension} declarations, including
 * those of included scope models, as dependencies of the model.
 * <p>
 ...
 */
@Singleton
public class ScopeResourceDescriptionManager extends XbaseResourceDescriptionManager {
  ...
  /**
   * Returns the lower-cased qualified names of the classes named in the {@code extension} declarations of the scope model in
   * the given resource and of the scope models it includes, in the form imported names are matched against changed types.
   *
   * @param resource
   *          the resource, must not be {@code null}
   * @return the names, never {@code null}
   */
  Set<QualifiedName> extensionClassNames(final Resource resource) {
    if (resource.getContents().isEmpty()) {
      return ImmutableSet.of();
    }
    final EObject root = resource.getContents().get(0);
    if (!(root instanceof ScopeModel model)) {
      return ImmutableSet.of();
    }
    final Set<QualifiedName> names = new LinkedHashSet<>();
