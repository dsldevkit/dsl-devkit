...
class ScopeNameProviderGenerator {
   ...
   */
  def void configure(GenModelUtilX genModelUtil, ScopeModel model) {
  ...
  def dispatch String nameFunction(NamingExpression it, ScopeModel model, String contextName, EClass contextType) {
    if (factory) {
      if (contextName === null || contextType === null) {
        ...
        compiler.javaExpression(expression, translator.newCompilationContext('UNEXPECTED_THIS', null, #[contextName -> genModelUtil.instanceClassName(contextType)], it))
      }
    } else if (export) {
  ...
  def dispatch String nameFunction(FeatureCall it, ScopeModel model, String contextName, EClass contextType) '''
    «val currentContext = if (contextName === null) translator.newCompilationContext('obj', scopeType(), #[], it) else translator.newCompilationContext('obj', scopeType(), #[contextName -> genModelUtil.instanceClassName(contextType)], it)»
