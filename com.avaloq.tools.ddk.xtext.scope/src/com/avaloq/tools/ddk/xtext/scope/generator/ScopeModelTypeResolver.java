...
public final class ScopeModelTypeResolver {
  ...
  public ScopeModelTypeResolver(final ScopeModel model) {
    this.model = model;
    this.packages = StreamSupport.stream(EObjectUtil.getScopeProviderByEObject(model).getScope(model, ScopePackage.Literals.IMPORT__PACKAGE).getAllElements().spliterator(), false)//
