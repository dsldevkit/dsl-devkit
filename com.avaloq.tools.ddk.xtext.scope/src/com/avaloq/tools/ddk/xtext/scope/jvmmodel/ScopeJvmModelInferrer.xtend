...
class ScopeJvmModelInferrer extends AbstractModelInferrer {
  ...
  def private void addExpressionMethod(JvmGenericType type, ScopeModel element, ScopeExpressionMethodRequest request) {
    ...
    val translationContext = new ScopeTranslationContext
    translationContext.sourceElement = element
    translationContext.putVariable(request.variableName, parameter)
    translationContext.implicitVariable = parameter
    translationContext.typeResolver = [Identifier t|element.newTypeRef(t.id.join('.'))?.type]
    ...
    for (extra : request.extraParameters) {
      ...
      translationContext.putVariable(extra.key, extraParam)
    }
    val translated = translator.translate(request.expression, translationContext)
    val returnType = if (request.returnTypeName == 'boolean') {
        ...
        val resolved = translator.resolveType(request.expression, translationContext)
        if (resolved !== null) typeRef(resolved) else typeRef(Object)
      }
