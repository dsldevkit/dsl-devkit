...
# 5) Skip every reactor module that was not touched by this PR. Kept bundles with
#    sources are expected to produce a SARIF; the gate checks each one.
kept=0
skipped=0
expect_reports=""
...
# Only source-less modules changed (feature / target / repository) counts as
# nothing to scan: SpotBugs cannot write a report for a bundle without classes.
if [ -z "$expect_reports" ]; then
  kept=0
fi
