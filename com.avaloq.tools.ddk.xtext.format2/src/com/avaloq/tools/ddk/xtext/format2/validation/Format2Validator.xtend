...
import com.avaloq.tools.ddk.xtext.format2.format2.SpecificDirective
import com.avaloq.tools.ddk.xtext.format2.format2.KeywordPair
...
import com.avaloq.tools.ddk.xtext.format2.format2.StringValue
 ...
 */
class Format2Validator extends AbstractFormat2Validator {
...
//					Format2Package.Literals.GREETING__NAME,
//					INVALID_NAME)
//		}
//	}
  ...
  def void checkDataTypeOrEnumRule(GrammarRule model) {
    if (model.getTargetRule() instanceof TerminalRule || model.getTargetRule() instanceof EnumRule
      ...
      if (!selfAccessOnly) {
        error(NLS.bind("For data type, enum or terminal rule {0} only ''rule'' directive may be used", model.getTargetRule().getName()), Format2Package::Literals::GRAMMAR_RULE__DIRECTIVES, ILLEGAL_DIRECTIVE_CODE)
      }
    }
  }
  ...
  def void checkRuleReference(GrammarElementReference elementReference) {
    ...
    if (directive.getMatcherList() !== null) {
      ...
      if (!twoArgumentMatcherTypesOnly) {
        error(NLS.bind("Grammar rules may only be used with \"{0}\" or \"{1}\"", MatcherType::RANGE.getName(), MatcherType::BETWEEN.getName()), Format2Package::Literals::GRAMMAR_ELEMENT_REFERENCE__RULE)
      }
      ...
      if (Iterables.size(ruleGrammarElements) != 1) {
        error(NLS.bind("Only one grammar rule may be referenced with \"{0}\" and \"{1}\"", MatcherType::RANGE.getName(), MatcherType::BETWEEN.getName()), Format2Package::Literals::GRAMMAR_ELEMENT_REFERENCE__RULE)
      }
    }
  }
  ...
  def void checkOverrideMissing(FormatConfiguration model) {
    ...
    for (GrammarRule overriddenRule : Iterables.filter(overriddenRules, GrammarRule)) {
      if (localAbstractRuleMap.containsKey(TARGET_RULE.apply(overriddenRule))) {
        ...
        error(OVERRIDE_MISSING_MESSAGE, localRule, Format2Package::Literals::GRAMMAR_RULE__TARGET_RULE, OVERRIDE_MISSING_CODE)
      }
    }
    // Check WildcardRule
  ...
  def void checkIllegalOverride(FormatConfiguration model) {
    ...
    for (GrammarRule overrideRule : Iterables.filter(overrideRules, GrammarRule)) {
      if (!overrideableAbstractRuleMap.containsKey(TARGET_RULE.apply(overrideRule))) {
        error(OVERRIDE_ILLEGAL_MESSAGE, overrideRule, Format2Package::Literals::GRAMMAR_RULE__TARGET_RULE, OVERRIDE_ILLEGAL_CODE)
      }
    }
    // Check WildcardRule
  ...
  def void checkExtendedGrammarCompatible(FormatConfiguration model) {
    ...
    error("Extended format configuration has incompatible grammar", Format2Package::Literals::FORMAT_CONFIGURATION__EXTENDED_FORMAT_CONFIGURATION, EXTENDED_GRAMMAR_INCOMPATIBLE_CODE)
  }
  ...
  def void checkRequiredRulesImplemented(FormatConfiguration model) {
    ...
    for (AbstractRule rule : model.getTargetGrammar().getRules()) {
      if (ruleNames.contains(rule.getName())) {
        error(NLS.bind("Required formatting for rule \"{0}\" missing", rule.getName()), Format2Package::Literals::FORMAT_CONFIGURATION__TARGET_GRAMMAR, GRAMMAR_RULE_MISSING_CODE, rule.getName())
      }
    }
  }
  ...
  def void checkIntValueConstantReference(IntValue value) {
    ...
    if (value.getReference().getIntValue() === null) {
      error(NLS.bind("Referenced const \"{0}\" is not an integer", value.getReference().getName()), Format2Package::Literals::INT_VALUE__REFERENCE)
    }
  }
  ...
  def void checkStringValueConstantReference(StringValue value) {
    ...
    if (value.getReference().getIntValue() === null) {
      error(NLS.bind("Referenced const \"{0}\" is not a string", value.getReference().getName()), Format2Package::Literals::STRING_VALUE__REFERENCE)
    }
  }
  ...
  def void checkConstantOptionalType(Constant value) {
    if (value.isIntType() && value.getIntValue() === null) {
      error("Value is not an integer", Format2Package::Literals::CONSTANT__NAME)
    } else if (value.isStringType() && value.getStringValue() === null) {
      error("Value is not a string", Format2Package::Literals::CONSTANT__NAME)
    }
  }
  ...
  private def void checkTwoArgumentMatcherType(Matcher matcher, MatcherType matcherType) {
    if (matcher.getType() == matcherType) {
      ...
      if (directive === null || directive.getGrammarElements().size() != 2) {
        error(NLS.bind("\"{0}\" may only be used with exactly two elements", matcherType.getName()), Format2Package::Literals::MATCHER__TYPE)
      }
    }
  }
