...
public final class ExportModelTypeResolver {
  ...
  public ExportModelTypeResolver(final ExportModel model) {
    this.model = model;
    this.packages = StreamSupport.stream(EObjectUtil.getScopeProviderByEObject(model).getScope(model, ExportPackage.Literals.IMPORT__PACKAGE).getAllElements().spliterator(), false)//
