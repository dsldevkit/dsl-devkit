...
| 28 | Infrastructure cleanup | Remove Xtend from MANIFEST.MF, build.properties, .classpath; delete `xtend-gen/` directory (when module fully off Xtend). Leave `.project` untouched — the Xtext builder/nature serve the module's grammar. |
...
- [ ] `mvn -pl :ddk-target,:<module> -am -DskipTests compile -f ./ddk-parent/pom.xml` — passes.
