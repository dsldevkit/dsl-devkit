...
9. **Verify the file compiles:**
   ```bash
   mvn -pl :<target-module>,:<module> -am -DskipTests compile -f <parent-pom> > mvn-output.txt 2>&1
   ```
10. **Run quality checks:**
    ```bash
    mvn -pl :<target-module>,:<module> -am checkstyle:check pmd:check -f <parent-pom> > mvn-output.txt 2>&1
    ```
