...
| **.project** | **Leave alone.** `org.eclipse.xtext.ui.shared.xtextBuilder` / `…xtextNature` belong to the module's own **grammar**, not to Xtend — removing them breaks the Xtext build. Only the `xtend-gen` classpath / build.properties entries and the `xtend-gen/` marker go. |
