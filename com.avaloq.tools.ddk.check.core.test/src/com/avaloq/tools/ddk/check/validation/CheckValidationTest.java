...
public class CheckValidationTest {
  ...
  private static final String FOR_X_ISSUE_A = "for X { issue A";
  private static final String FOR_C_C_ISSUE = "for C c {issue}";
  ...
  public void testImplicitIssuedCheck() throws Exception {
    final CheckCatalog model = parser.parse(modelUtil.modelWithCheck(CHECK_A) + FOR_X_ISSUE_A);
    helper.assertWarning(model, CheckPackage.Literals.XISSUE_EXPRESSION, IssueCodes.IMPLICIT_ISSUED_CHECK);
    final CheckCatalog model2 = parser.parse(modelUtil.modelWithCheck(CHECK_A) + FOR_X_ISSUE_A);
  ...
  public void testContextTypeIsUnique() throws Exception {
    // should fail
    List<String> contexts = Lists.newArrayList(FOR_C_C_ISSUE, "for C d {issue}");
    ...
    // should fail
    contexts = Lists.newArrayList(FOR_C_C_ISSUE, "for D d {issue}", "for C e {issue}");
