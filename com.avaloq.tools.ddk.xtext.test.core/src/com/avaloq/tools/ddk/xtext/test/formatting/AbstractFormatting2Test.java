 ...
 */
public abstract class AbstractFormatting2Test extends AbstractXtextTest {
  ...
  protected final void assertFormattedParseTreeConstructor() {
    assertFormattedParseTreeConstructor(getSemanticModel(), getTestSource().getContent(), getTestSource(getExpectedTestSourceFileName()).getContent());
  }
  ...
  protected final void assertPreservedParseTreeConstructor() {
    XtextTestSource testSource = getTestSource(getExpectedTestSourceFileName());
    assertFormattedParseTreeConstructor(testSource.getModel(), testSource.getContent(), testSource.getContent());
  }
   ...
   */
  private void assertFormattedParseTreeConstructor(final EObject model, final String input, final String expected) {
    FormatterTestRequest testRequest = new FormatterTestRequest().setRequest(new FormatterRequest());
    testRequest.setToBeFormatted(input).setExpectation(expected).setUseNodeModel(false);
    FormatterTestHelper testHelper = getXtextTestUtil().get(FormatterTestHelper.class);
    testHelper.assertFormatted(testRequest);
  ...
  private void assertFormattedNodeModel(final EObject model, final String input, final String expected, final int offset, final int length) {
    FormatterTestRequest testRequest = new FormatterTestRequest().setRequest(new FormatterRequest());
    testRequest.setToBeFormatted(input).setExpectation(expected).setUseSerializer(false);
    FormatterTestHelper testHelper = getXtextTestUtil().get(FormatterTestHelper.class);
    testHelper.assertFormatted(testRequest);
