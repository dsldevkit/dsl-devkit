...
import com.avaloq.tools.ddk.xtext.build.DdkBuildPhases;
 ...
 */
public class DdkResourceDescription extends DefaultResourceDescription implements IResourceDescription2 {

  private static final Logger LOG = Logger.getLogger(DdkResourceDescription.class);

  private final IDefaultResourceDescriptionStrategy strategy;

  public DdkResourceDescription(final Resource resource, final IDefaultResourceDescriptionStrategy strategy, final IResourceScopeCache cache) {
  ...
  protected List<IEObjectDescription> computeExportedObjects() {
    ...
    boolean doInitialize = (resource instanceof IDdkLazyLinkingResource) && !((IDdkLazyLinkingResource) resource).isInitialized();
    final DdkResourceDescription self = this;
    try {
      if (doInitialize) {
        resource.eSetDeliver(false);
        ((IDdkLazyLinkingResource) resource).installDerivedState(DdkBuildPhases.isIndexing(resource));
        // Make sure we have at least this resource description itself in the resource cache.
        resource.getCache().get(AbstractDdkResourceDescriptionManager.CACHE_KEY, resource, new Provider<DdkResourceDescription>() {
          @Override
          public DdkResourceDescription get() {
            return self;
          }
        });
      }
  ...
  protected List<IReferenceDescription> computeReferenceDescriptions() {
    ...
    if (strategy instanceof AbstractDdkResourceDescriptionStrategy) {
      ((AbstractDdkResourceDescriptionStrategy) strategy).createImplicitReferenceDescriptions(getResource(), acceptor);
    }
  ...
  private boolean isExportedEClass(final EClass type) {
    if (type == EcorePackage.Literals.EOBJECT || !(strategy instanceof AbstractDdkResourceDescriptionStrategy)) {
    ...
    Set<EClass> exportedEClasses = ((AbstractDdkResourceDescriptionStrategy) strategy).getExportedEClasses(getResource());
