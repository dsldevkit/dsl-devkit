 ...
 */
public abstract class AbstractDdkResourceDescriptionManager extends DerivedStateAwareResourceDescriptionManager implements IResourceDescriptionDeltaFactory {

  /** Class-wide logger. */
  private static final Logger LOGGER = Logger.getLogger(AbstractDdkResourceDescriptionManager.class);
  ...
  protected IResourceDescription createResourceDescription(final Resource resource, final IDefaultResourceDescriptionStrategy strategy) {
    return new DdkResourceDescription(resource, strategy, getCache());
  }
  ...
  public Collection<URI> getAffectedResources(final Collection<Delta> deltas, final Collection<URI> candidates, final IResourceDescriptions context) {
    ...
    // work precisely. Second, it filters by haveEObjectdescriptionsChanged(), but the DdkClusteringBuilderState already ensures that
    ...
    // DdkClusteringBuilderState be effective.
    //
    // See also the comment on DdkClusteringBuilderState.FindReferenceCachingState.
  ...
  protected Set<URI> getAffectedResources(final Collection<Delta> deltas, final Predicate<URI> filter, final IResourceDescriptions2 context) { // NOPMD
    ...
    for (Delta delta : deltas) {
      if (delta.haveEObjectDescriptionsChanged()) {
        if (delta instanceof DdkResourceDescriptionDelta && ((DdkResourceDescriptionDelta) delta).hasObjectFingerprints()) {
          DdkResourceDescriptionDelta detailedDelta = (DdkResourceDescriptionDelta) delta;
  ...
  public IResourceDescription.Delta createDelta(final IResourceDescription oldState, final IResourceDescription newState) {
    return new DdkResourceDescriptionDelta(oldState, newState, index);
  }
