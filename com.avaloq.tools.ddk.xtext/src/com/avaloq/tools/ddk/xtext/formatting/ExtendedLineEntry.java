...
import com.avaloq.tools.ddk.xtext.formatting.ExtendedFormattingConfigBasedStream.AbstractExtendedLineEntry;
 ...
 */
public class ExtendedLineEntry extends AbstractExtendedLineEntry {
  ...
  final String value, final boolean isHidden, final Set<ElementLocator> beforeLocators, final String leadingWS, final int indent, final ParserRule hiddenTokenDefinition) {
    ...
    this.acsLocators = Sets.newHashSet(Iterables.filter(beforeLocators, IExtendedLocator.class));
  }
  ...
  /**
   * Checks if formatting is disabled for this {@link ExtendedLineEntry}.
   *
  ...
  protected Set<IExtendedLocator> getEntryLocators() {
    return acsLocators;
  }
  ...
  public boolean containsVariableLocator() {
    for (IExtendedLocator acsLocator : acsLocators) {
      if (acsLocator instanceof ColumnLocator || acsLocator instanceof OffsetLocator) {
        return true;
      }
    }
    return false;
  ...
  public void setColumnIndent(final int column, final int indent) {
    if (isRelative()) {
      this.columnIndent = column + indent * configBasedStream.getFormattingConfig().getIndentationSpace().length();
    } else {
