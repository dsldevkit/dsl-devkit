 ...
 */
public class DdkLine extends DdkFormattingConfigBasedStream.AbstractDdkLine {

  /**
   * All entries are contained in this list.
   */
  private final List<DdkLineEntry> lineEntries = Lists.newArrayList();
  /**
  ...
  /**
   * Creates a new instance of {@link com.avaloq.tools.ddk.xtext.formatting.DdkFormattingConfigBasedStream.AbstractDdkLine}.
   */
  public DdkLine(final DdkFormattingConfigBasedStream formattingConfigBasedStream) { // NOPMD - false positive by PMD
    formattingConfigBasedStream.super();
   ...
   */
  protected boolean isFirstEntry(final DdkLineEntry entry) {
    return getEntries().indexOf(entry) == 0;
  }

  @Override
  public DdkLine addEntry(final DdkLineEntry lineEntry) throws IOException {
   ...
   */
  protected boolean mustFormatLineEntry(final DdkLineEntry lineEntry) {
    return !(isPreserveSpaces() && lineEntry.getPreservedWS() != null);
  }

  /**
   * Checks whether the given {@link DdkLineEntry} is a single line comment.
   *
   * @param entry
   *          the entry to check
   * @return {@code true} if the entry is an SL-Comment, otherwise {@code false}
   */
  protected boolean isSLComment(final DdkLineEntry entry) {
  ...
  /**
   * Checks whether the given {@link DdkLineEntry} is a multiline comment.
   *
   * @param entry
   *          the entry to check
   * @return {@code true} if the entry is a ML-Comment, otherwise {@code false}
   */
  protected boolean isMLComment(final DdkLineEntry entry) {
    if (isCommentEntry(entry)) {
      final AbstractRule grammarElement = (AbstractRule) entry.getGrammarElement();
   ...
   */
  protected boolean isCommentEntry(final DdkLineEntry lineEntry) {
   ...
   */
  private boolean isTextField(final DdkLineEntry lineEntry) {
    final EObject grammarElement = lineEntry.getGrammarElement();
   ...
   */
  private boolean isIgnorableEntry(final DdkLineEntry lineEntry) {
   ...
   */
  protected DdkLine formatLineEntry(final DdkLineEntry lineEntry) throws IOException {
    DdkLine newLine = this;
    final WrapEntry wrapEntry = processLinewraps(lineEntry);
    if (!isLineEmpty() && wrapEntry.isWrap()) {
      flush();
      return ((DdkLine) formattingConfigBasedStream.createLine()).addEntry(lineEntry);
    } else if (isLineEmpty()) {
      ...
      if (isIgnorableEntry(lineEntry)
          || len + spaceEntry.getLength() + lineEntry.countCharactersInFirstLine() <= (formattingConfigBasedStream.getCharsPerLine())
    ...
    if (lastLineLength >= 0) {
      flush();
      newLine = (DdkLine) formattingConfigBasedStream.createLine();
      if (lastLineLength > 0) {
        newLine.setInitialLinewrapLocator();
      } else {
        newLine.setInitialLinewrapLocator(new SubtractingLinewrapLocatorFacade(formattingConfigBasedStream.getFormattingConfig()));
      }
    }
    return newLine;
  }
   ...
   */
  protected DdkLine preserveFormat(final DdkLineEntry lineEntry) {
   ...
   */
  public int getColumn(final DdkLineEntry entry) {
   ...
   */
  public int getOffset(final DdkLineEntry entry) {
    int column = 0;
    for (DdkLineEntry lineEntry : getEntries()) {
      if (entry == lineEntry) {
        break;
      }
   ...
   */
  public int getAbsoluteLineLength(final DdkLineEntry entry) {
    int lineLength = 0;
    int previousEntryIndex = findPredecessorOrLastEntryIndex(entry);

    if (entry != null) {
      DdkLineEntry previousEntry = getEntries().get(previousEntryIndex);
   ...
   */
  private boolean existsIndirectColumnAlignedPredecessor(final DdkLineEntry entry, final DdkLineEntry previousEntry) {
    for (DdkLineEntry lineEntry : lineEntries) {
   ...
   */
  public int findPredecessorOrLastEntryIndex(final DdkLineEntry entry) {
    int index = 0;
  ...
  protected void flush(final ITokenStream intoStream, final int endIndex) throws IOException {
    ...
    for (int i = firstNotIgnored; i < endIndex; i++) {
      final DdkLineEntry entry = getEntries().get(i);
      int previousEntryIndex = findPredecessorOrLastEntryIndex(entry);
      final DdkLineEntry previousEntry = lineEntries.get(previousEntryIndex);
   ...
   */
  protected DdkLine breakLine() throws IOException {
    final DdkLineEntry lastBreakableEntry = getLastBreakableLineEntry();
    DdkLine newLine = null;
    if (lastBreakableEntry != null) {
      ...
      newLine = (DdkLine) formattingConfigBasedStream.createLine();
      newLine.setInitialLinewrapLocator();
      final List<DdkLineEntry> lineEntries = getEntries();
      for (int i = lastBrakableIndex; i < lineEntries.size(); i++) {
        newLine = newLine.addEntry(lineEntries.get(i));
      }
   ...
   */
  public SpaceEntry processSpaces(final DdkLineEntry entry) {
   ...
   */
  protected WrapEntry processLinewraps(final DdkLineEntry lineEntry) {
   ...
   */
  public List<DdkLineEntry> getEntries() {
    return lineEntries;
  }
   ...
   */
  protected DdkLineEntry getLastBreakableLineEntry() {
   ...
   */
  public DdkLineEntry getPrecedingLineEntry(final DdkLineEntry entry) {
    DdkLineEntry result = null;
    for (DdkLineEntry le : getEntries()) {
      if (le != entry) {
        result = le;
      } else {
        return result;
  ...
  void setInitialLinewrapLocator() {
    setInitialLinewrapLocator(new LinewrapLocatorFacade(formattingConfigBasedStream.getFormattingConfig(), 1));
  }
