...
public class FormatValidator extends AbstractFormatValidator {
  ...
  public void checkDataTypeOrEnumRule(final GrammarRule model) {
    if (model.getTargetRule() instanceof TerminalRule || model.getTargetRule() instanceof EnumRule
      ...
      final Iterator<EObject> grammarElementAccessors = collectGrammarElementAccessors(model);
      final boolean selfAccessOnly = Iterators.all(grammarElementAccessors, new Predicate<EObject>() {
        @Override
  ...
  public void checkRuleReference(final GrammarElementReference elementReference) {
    ...
    final SpecificDirective directive = EcoreUtil2.getContainerOfType(elementReference, SpecificDirective.class);
    if (directive.getMatcherList() != null) {
      final boolean twoArgumentMatcherTypesOnly = Iterables.all(directive.getMatcherList().getMatchers(), new Predicate<Matcher>() {
        @Override
        public boolean apply(final Matcher input) {
          return Objects.equals(input.getType(), MatcherType.BETWEEN) || Objects.equals(input.getType(), MatcherType.RANGE);
        }
      });
      ...
      final Iterable<GrammarElementReference> ruleGrammarElements = Iterables.filter(directive.getGrammarElements(), new Predicate<GrammarElementReference>() {
        @Override
  ...
  public void checkOverrideMissing(final FormatConfiguration model) {
    final FormatConfiguration extendedModel = model.getExtendedFormatConfiguration();
    ...
    // Check GrammarRules
    for (final GrammarRule overriddenRule : Iterables.filter(overriddenRules, GrammarRule.class)) {
      if (localAbstractRuleMap.containsKey(TARGET_RULE.apply(overriddenRule))) {
        final GrammarRule localRule = localAbstractRuleMap.get(TARGET_RULE.apply(overriddenRule));
  ...
  public void checkIllegalOverride(final FormatConfiguration model) {
    final Iterable<Rule> overrideRules = Iterables.filter(model.getRules(), IS_OVERRIDE);
    ...
    // Check GrammarRules
    for (final GrammarRule overrideRule : Iterables.filter(overrideRules, GrammarRule.class)) {
  ...
  public void checkRequiredRulesImplemented(final FormatConfiguration model) {
    final FormatConfiguration extendedModel = model.getExtendedFormatConfiguration();
    if (extendedModel == null || extendedModel.eIsProxy() || Objects.equals(model.getTargetGrammar(), extendedModel.getTargetGrammar())) { // NOPMD
      return;
    }
    ...
    for (final GrammarRule rule : Iterables.filter(model.getRules(), GrammarRule.class)) {
      if (rule.getTargetRule() != null && !rule.getTargetRule().eIsProxy()) {
        ruleNames.remove(rule.getTargetRule().getName());
      }
    }
    for (final AbstractRule rule : model.getTargetGrammar().getRules()) {
  ...
  private void checkTwoArgumentMatcherType(final Matcher matcher, final MatcherType matcherType) {
    if (Objects.equals(matcher.getType(), matcherType)) {
      final SpecificDirective directive = EcoreUtil2.getContainerOfType(matcher, SpecificDirective.class);
