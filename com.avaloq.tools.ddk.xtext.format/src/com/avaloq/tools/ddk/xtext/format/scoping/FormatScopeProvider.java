...
public class FormatScopeProvider extends AbstractFormatScopeProvider {
   ...
   *
   * @param context
   *          the grammar to start from
   * @return the grammar and all its transitive base grammars
   */
  public Iterable<Grammar> getUsedGrammar(final Grammar context) {
    ...
    if (usedGrammars != null && !usedGrammars.isEmpty()) {
      Iterables.addAll(grammars, Iterables.concat(Iterables.transform(usedGrammars, g -> getUsedGrammar(g))));
    }
    return grammars;
  }
   ...
   *
   * @param context
   *          an object contained in the formatter
   * @return the target grammar and all its transitive base grammars
   */
   ...
   *
   * @param context
   *          the format configuration to start from
   * @return the format configuration and all its transitively extended configurations
   */
   ...
   * @param context
   *          an object contained in the formatter
   * @return the grammars ordered along the inheritance/overriding path
   */
   ...
   * @param context
   *          an object contained in the formatter
   * @return the format configurations ordered along the inheritance/overriding path
   */
   ...
   *
   * @param parent
   *          the parent scope to chain onto, or {@code null} to start a new chain
   * @param rulesForGrammars
   *          the rules grouped per grammar
   * @return the chained scope covering all given rules
   */
  public IScope createScopeForAbstractRules(final IScope parent, final Iterable<EList<AbstractRule>> rulesForGrammars) {
    if (parent == null) {
      ...
      if (Iterables.isEmpty(rulesForGrammars)) {
        ...
        return createScopeForAbstractRules(
            MapBasedScope.createScope(parent, createDescriptions(Iterables.getFirst(rulesForGrammars, null))), Iterables.skip(rulesForGrammars, 1));
      }
    }
  }
   ...
   *          the element type
   * @param elements
   *          the elements to create a scope for
   * @return the scope covering the given elements
   */
   ...
   *
   * @param description
   *          the object description to wrap in a scope
   * @return the simple scope containing the given description
   */
   ...
   *          the compound elements to create a scope for
   * @return the scope covering the given compound elements
   */
   ...
   *
   * @param elements
   *          the format configurations to describe
   * @return the object descriptions for the given format configurations
   */
  public Collection<IEObjectDescription> createDescriptionsFormats(final List<FormatConfiguration> elements) {
    return Collections2.transform(elements, e -> EObjectDescription.create(scopeUtil.findTargetGrammar(e).getName(), e));
  }
   ...
   * @return the object descriptions for the given objects
   */
  public Collection<IEObjectDescription> createDescriptions(final List<? extends EObject> elements) {
    ...
    return Collections2.transform(elements, e -> EObjectDescription.create(namingFunction.apply(e), e));
  }
   ...
   *
   * @param elements
   *          the compound elements to describe
   * @return the object descriptions for the given compound elements
   */
