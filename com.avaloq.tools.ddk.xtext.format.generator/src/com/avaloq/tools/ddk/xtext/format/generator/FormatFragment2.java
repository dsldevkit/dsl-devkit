...
public class FormatFragment2 extends AbstractStubGeneratingFragment {
  ...
  public void generate() {
    ...
    final ManifestAccess manifest = getProjectConfig().getRuntime().getManifest();
    if (manifest != null) {
      manifest.getRequiredBundles().add("org.eclipse.emf.ecore");
      manifest.getRequiredBundles().add(RUNTIME_PLUGIN);
    }
    final ManifestAccess eclipsePluginManifest = getProjectConfig().getEclipsePlugin().getManifest();
    if (eclipsePluginManifest != null) {
      eclipsePluginManifest.getRequiredBundles().add(RUNTIME_PLUGIN);
    }
    final ManifestAccess genericIdeManifest = getProjectConfig().getGenericIde().getManifest();
