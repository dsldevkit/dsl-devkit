...
public class GrammarRuleAnnotations {
   ...
   */
  private static final Pattern NO_BACKTRACK_ANNOTATION_PATTERN = Pattern.compile("@NoBacktrack", Pattern.MULTILINE);
  ...
  public static class GrammarAnnotations {
    ...
    public int hashCode() {
      final int prime = 31;
      return prime * 1 + ((this.predicates == null) ? 0 : this.predicates.hashCode());
    }
  ...
  public GrammarAnnotations annotateGrammar(final Grammar grammar) {
    ...
    if (annotations == null) {
      annotations = new GrammarAnnotations(grammar.getRules().stream().map(r -> annotateRule(r)).filter(a -> a != null).collect(Collectors.toList()));
      annotations.attachToEmfObject(grammar);

      Set<Grammar> inheritedGrammars = allInheritedGrammars(grammar);
      for (final Grammar inheritedGrammar : inheritedGrammars) {
        GrammarAnnotations inheritedAnnotations = GrammarAnnotations.findInEmfObject(inheritedGrammar);
        if (inheritedAnnotations == null) {
          inheritedAnnotations = new GrammarAnnotations(inheritedGrammar.getRules().stream().map(r -> annotateRule(r)).filter(a -> a != null).collect(Collectors.toList()));
  ...
  public Set<Grammar> allInheritedGrammars(final Grammar grammar) {
    ...
    for (final AbstractRule rule : GrammarUtil.allParserRules(grammar)) {
      Grammar ruleGrammar = GrammarUtil.getGrammar(rule);
      if (!ruleGrammar.equals(grammar) && isSemanticPredicate(rule)) {
  ...
  public AbstractElement getFirstNonActionElement(final Group group) {
    for (final AbstractElement groupElement : group.getElements()) {
  ...
  public boolean isAlternativeAvailable(final AbstractElement element) {
    ...
    if (container instanceof Assignment) {
      ...
      for (final AbstractElement alternative : ((Alternatives) container).getElements()) {
        if (!Objects.equals(alternative, element) /* && !isGated(alternative) */) {
          return true;
        }
      }
    }
  ...
  public String generateGatedPredicate(final SemanticPredicate predicate) {
    return "{predicates." + predicate.getName() + "(parserContext)}?=>";
  }

  public String generateValidatingPredicate(final SemanticPredicate predicate) {
    return "{predicates." + predicate.getName() + "(parserContext) /* @ErrorMessage(" + predicate.getMessage() + ") */}?";
  }
   ...
   *
   * @param rule
   *          the grammar rule to annotate
   * @return the attached {@link SemanticPredicate}, or {@code null} if the rule has no predicate annotation
   * @throws IllegalArgumentException
   *           if a keyword annotation is combined with a semantic predicate annotation on the same rule
   */
  ...
  public SemanticPredicate getSemanticPredicateAnnotation(final AbstractRule rule) {
    if (rule != null) {
      RuleWithParameterValues ruleWithParameterValues = RuleWithParameterValues.findInEmfObject(rule);
      AbstractRule original = ruleWithParameterValues != null ? ruleWithParameterValues.getOriginal() : null;
      if (original == null) {
        original = rule;
      }
   ...
   *
   * @param text
   *          the rule text to inspect
   * @param ruleName
   *          the name of the rule
   * @param grammar
   *          the simple name of the grammar containing the rule
   * @return the corresponding {@link SemanticPredicate}, or {@code null} if no keyword rule annotation is present
   */
   ...
   * @param grammar
   *          the simple name of the grammar containing the rule
   * @return the corresponding {@link SemanticPredicate}, or {@code null} if no semantic predicate annotation is present
   */
   ...
   * @param text
   *          the rule text to inspect
   * @return the corresponding {@link NoBacktrack}, or {@code null} if no NoBacktrack annotation is present
   */
  ...
  public String getText(final AbstractRule rule) {
    ...
    return node != null ? node.getText() : grammarExtensions.grammarFragmentToString(rule, "");
  }
