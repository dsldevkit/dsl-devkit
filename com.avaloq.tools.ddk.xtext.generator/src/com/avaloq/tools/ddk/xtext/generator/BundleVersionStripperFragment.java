...
public class BundleVersionStripperFragment extends AbstractXtextGeneratorFragment {
  ...
  public void generate() {
    for (final IBundleProjectConfig project : Iterables.filter(getProjectConfig().getEnabledProjects(), IBundleProjectConfig.class)) {
      final ManifestAccess manifest = project.getManifest();
      if (manifest != null) {
  ...
  public void stripBundleVersions(final ManifestAccess manifest) {
    for (final String bundleName : bundles) {
