...
(* PLANTED BUG (toProcess never incremented). Derived from the FIXED COPY. Differences from ParallelLoader.tla, marked FIX-n:         *)
...
Init ==
  ...
  /\ op = 1
  /\ toProcess = 0 \* PLANTED BUG: PRL:272 dropped load(): toProcess += uris.size()
...
WPut(e, w) ==
  /\ wst[e][w].k = "put"
  /\ IF wintr[e][w] \/ cancelled[e]                \* FIX-3
