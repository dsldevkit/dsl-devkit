...
(* ABLATION: FIX-1 undone. Derived from the FIXED COPY. Differences from ParallelLoader.tla, marked FIX-n:         *)
...
PollTimeout ==
  ...
  /\ ~bintr
  /\ ~Available(op)
  /\ toProcess' = toProcess - 1 \* ABLATED FIX-1
  /\ pc' = "catch"
  /\ exc' = "timeout"
  /\ UNCHANGED <<queue, remaining, op, pending, wst, wintr, resQ, cancelled, bintr, nbintr, cancelReq>>
